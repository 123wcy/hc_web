		$(function(){
			...
			// alert($("#jsnumber").val());
			$("#jsnumber").val(Request["expressNumber"]);
			...
			$.ajax({
				...
				url: "http://58.221.6.82:900/goods/findDetailsByExpressNumber?expressNumber=" + expressNumber,
	
				success: function (res) {
					var jsonobject = res.data;
					// alert(JSON.stringify(res));
					var goods = jsonobject.goods;
					//goods是否为空判断
					if (goods == null) {
						alert("goods为空！");
					} else {
						var completed = "";
						// alert(goods.completed);
						if (goods.completed == "Y") {
							completed = "已签收";
						} else {
							completed = "未签收";
						}
						...
						var html = '<div class="bill-address mt10"><span>' + goods.senderAddress +
							'</span><img src="../../img/jiantou.png" alt=""><span>' + goods.recipientAddress +
							'</span></div><p>' + goods.expressNumber + '</p><div class="bill-time"><label>' + goods
							.createtime + '</label><span>' + completed + '</span></div>'
						$('#waybill').append(html);
					}
	
	
					var jsonarray = jsonobject.operations; //从jsonobject获取operations（operations是jsonArray）
					//operations是否为空判断
					if (jsonarray == null) {
						alert("operations为空！");
					} else {
						...
						for (var i in jsonarray) {
							var obj = jsonarray[i]; //jsonarray数组第i个jsonobject
							...
							if (week == sameday) {
								result += '<li>' + hms + '<span>' + tab + '</span></li>';
							} else {
								// if( i == 1 ){
								// 	result+='<li><div class="time-bg clearfix"><span>'+ymd+'  '+week+'</span><i><img src="../../img/pulldown.png" alt=""></i></div><ul>';
								// 	result+='<li>'+hms+'<span>'+tab+'</span></li>';
								// }else{
								result += '</ul></li>';
								result += '<li><div class="time-bg clearfix"><span>' + ymd + '  ' + week +
									'</span><i><img src="../../img/pulldown.png" alt=""></i></div><ul>';
								result += '<li>' + hms + '<span>' + tab + '</span></li>';
								// }
							}
						...
						$('#list_page').append(result.substring(10, result.length) + '</ul></li>');
					}
				},
				error: function () {
					console.log("这个请求错误");
			...
			methods: {
				...
				submit() {
					...
					$.ajax({
						...
						url: "http://58.221.6.82:900/express/add",
						data: JSON.stringify(list),
						success: function (res) {
							// var str = JSON.stringify(res);
							// alert(str);
