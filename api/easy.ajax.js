...
(function (window, $) {
            ...
            success: function (_resultData) {
                ...
                if(_resultData.success==true || config.mustCallback){
                    (callback && typeof(callback) === "function") && callback(_resultData);
                   	...
                   	if(callbackError && typeof(callbackError) === "function"){ // 有error function先执行function中的内容
                   		callbackError(_resultData);
                   	}else{
                   		$.message(_resultData.message);
                   	}
                }
            },
    ...
    function _handleStatus(status) {
        switch (status) {
            case 404:
            	$.message('请求资源不存在：#' + status, "text");
                break;
            case 400:
            	$.message('请求参数有误：#' + status, "text");
                break;
            case 500:
            	$.message('服务器错误：#' + status, "text");
                break;
            case 504:
            	$.message('链接超时：#' + status, "text");
                break;
//          case 401:
//          	window.top.location.href='/login.html';//整体跳转iframe 页面跳转到登陆界面
//          	break;
            default:
            	$.message('服务器异常：#' + status, "text");
                break;
        }
    }
